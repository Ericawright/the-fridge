...
Bake in the oven for 30-45 minutes, taking off the aluminum foil for the last 10.", picture: '#').ingredients << Ingredient.find(2) <<Ingredient.find(3) << Ingredient.find(4) << Ingredient.find(5) << Ingredient.find(8) << Ingredient.find(9) << Ingredient.find(13)
...
Bake for 30 to 35 minutes, or until chicken is no longer pink. Remove from oven, and place 1/2 cheese slice on top of each breast. Return to oven for 3 to 5 minutes, or until cheese has melted. Remove toothpicks, and serve immediately.
 ", picture: '#').ingredients << Ingredient.find(2) << Ingredient.find(6) << Ingredient.find(4) << Ingredient.find(8) << Ingredient.find(9) << Ingredient.find(19)
...
Cook under the preheated broiler until the fish flakes easily with a fork, 10 to 15 minutes.", picture:'#').ingredients << Ingredient.find(8) << Ingredient.find(9) << Ingredient.find(10) << Ingredient.find(11) << Ingredient.find(12)
...
Bake at 400 degrees F (200 degrees C) for 30 minutes.", picture: '#').ingredients << Ingredient.find(2) << Ingredient.find(12) << Ingredient.find(14) << Ingredient.find(15) << Ingredient.find(16)
 ...
 Melt butter in a large skillet over medium high heat. Add chicken and sprinkle with garlic powder, seasoning salt and onion powder. Saute about 10 to 15 minutes on each side, or until chicken is cooked through and juices run clear.", picture: '#').ingredients << Ingredient.find(2) << Ingredient.find(13) << Ingredient.find(8) << Ingredient.find(17) << Ingredient.find(18)
...
Bake in a preheated oven for 15 minutes, or until salmon flakes easily with a fork.", picture: '#').ingredients << Ingredient.find(2) << Ingredient.find(12) << Ingredient.find(14) << Ingredient.find(15) << Ingredient.find(16)
...
Roast in the preheated oven until squash is tender and lightly browned, 25 to 30 minutes.", picture: '#').ingredients << Ingredient.find(20) << Ingredient.find(21) << Ingredient.find(22) << Ingredient.find(8) << Ingredient.find(9)
 
 Recipe.create!(name: "Fried Cabbage", cook_time: 2700, body: "1 pound bacon, diced
 2 onions, chopped
 1 cup diced bell pepper
 1 head cabbage, chopped
 Cook and stir bacon, onions, and bell pepper together in a skillet over medium heat until bacon is just starting to brown, about 10 minutes. Add cabbage and cook, stirring frequently, until cabbage is tender and transparent, 20 to 30 minutes.", picture: '#').ingredients << Ingredient.find(23) << Ingredient.find(24) << Ingredient.find(25) << Ingredient.find(26)
